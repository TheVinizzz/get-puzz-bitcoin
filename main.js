...
import encontrarBitcoins, {encontrarBitcoinsRandom} from './bitcoin-find.js'
...
console.log(process.env.TYPE_EXEC)
...
if(process.env.TYPE_EXEC === "forever") {
    let answer4 = process.env.RANGE
    let key = process.env.HASH_CODE
    let min = key
    console.log(chalk.cyan("Rodando em forever"))
    encontrarBitcoinsRandom(key, min, max, () => shouldStop, answer4)
} else {
        ...
        rl.question(`Escolha uma opcao (${chalk.cyan(1)} - Comecar do inicio, ${chalk.cyan(2)} - Escolher uma porcentagem, ${chalk.cyan(3)} - Escolher minimo): `, (answer2) => {
            if (answer2 == '2'){
                ...
                rl.question('Entre o minimo: ', (answer3) => {
                    min = answer3
                    key = answer3
                    rl.question('Qual Range de caracteres: ', (answer4) => {
                        encontrarBitcoinsRandom(key, min, max, () => shouldStop, answer4)
                        rl.close();
                    })
                });
